...
export default function Admin() {
  return (
    <section className="min-h-screen bg-[#eef5ef] px-3 pb-8 pt-3 sm:px-5 lg:px-6">
      {/* MOBILE ADMIN HEADER NAV (VISIBLE ONLY ON MOBILE) */}
      <div className="mb-4 rounded-2xl border border-emerald-900/10 bg-slate-950 p-3.5 text-white shadow-md lg:hidden">
        ...
        {/* Horizontal Scrollable Menu for Mobile Admin */}
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {stats.map(({ icon: Icon, value, label, change }) => (
              <div key={label} className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-5 text-2xl sm:text-3xl font-black text-slate-950">{value}</p>
                <p className="mt-1 text-xs sm:text-sm font-bold text-slate-600">{label}</p>
              </div>
            ))}
          </div>
            ...
            <div className="space-y-5">
              ...
              <section id="antrian-layanan" className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.16em] text-emerald-700">Antrian kerja</p>
                    <h2 className="mt-1 text-xl sm:text-2xl font-black text-slate-950">Butuh perhatian operator</h2>
                  </div>
                  <Link href="/layanan" className="text-xs sm:text-sm font-bold text-emerald-700">
                    Lihat semua
                  </Link>
                </div>
              ...
              <section id="modul-cms" className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                {modules.map(({ icon: Icon, title, description, href }) => (
                  <Link key={title} href={href} className="group rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg">
                    ...
                    <p className="mt-2 text-xs sm:text-sm leading-6 text-slate-500">{description}</p>
                    <span className="mt-5 inline-flex items-center gap-1 text-xs sm:text-sm font-bold text-emerald-700">
            ...
            <aside className="space-y-5">
              <section id="ringkasan-cms" className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm font-bold uppercase tracking-[0.16em] text-emerald-700">Hari ini</p>
                ...
                <div className="mt-5 space-y-3">
                  {schedules.map((item, index) => (
                    <div key={item} className="flex gap-3 rounded-2xl bg-slate-50 p-3">
                      ...
                      <p className="text-xs sm:text-sm font-semibold leading-6 text-slate-700">{item}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section id="status-sistem" className="rounded-[28px] border border-emerald-900/10 bg-slate-950 p-5 text-white shadow-lg">
                ...
                <p className="mt-5 text-xs sm:text-sm font-bold uppercase tracking-[0.16em] text-emerald-300">Ringkasan</p>
                <h2 className="mt-2 text-xl sm:text-2xl font-black">96% layanan selesai</h2>
                <div className="mt-5 space-y-4">
                  {[
                    ...
                    <div key={label as string}>
                      <div className="mb-2 flex justify-between text-xs sm:text-sm font-semibold text-slate-300">
                        <span>{label}</span>
                        <span>{value}%</span>
                      </div>
