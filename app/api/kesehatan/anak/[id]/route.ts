...
export async function PATCH(request: Request, context: RouteContext<"/api/kesehatan/anak/[id]">) {
  ...
  try { const { id } = await context.params; return Response.json(await prisma.child.update({ where: { id }, data: validateChildUpdate(await request.json()) })) }
...
export async function DELETE(_: Request, context: RouteContext<"/api/kesehatan/anak/[id]">) {
  ...
  try { const { id } = await context.params; await prisma.$transaction(async (tx) => { await tx.childHealthCheck.deleteMany({ where: { childId: id } }); await tx.child.delete({ where: { id } }) }); return new Response(null, { status: 204 }) }
