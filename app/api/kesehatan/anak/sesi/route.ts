...
export async function POST(request: Request) { const user = await getCurrentHealthUser(); if (!user) return Response.json({ error: "Silakan masuk untuk mengakses data kesehatan." }, { status: 401 }); try { const session = await prisma.childPosyanduSession.create({ data: { ...validateChildSessionInput(await request.json()), createdById: user.id }, include: { createdBy: { select: { name: true, username: true } }, _count: { select: { checks: true } } } }); await publishCmsUpdate("health-child"); return Response.json(session, { status: 201 }) } catch (error) { return Response.json({ error: error instanceof Error ? error.message : "Sesi posyandu anak tidak valid." }, { status: 400 }) } }
