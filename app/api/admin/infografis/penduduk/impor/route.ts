...
export async function POST(request: Request) {
  ...
  try { const result = await prisma.resident.createMany({ data: valid }); revalidateTag("home-data", "max"); revalidatePath("/infografis"); await publishCmsUpdate("population"); return Response.json({ created: result.count, errors: [] }) }
