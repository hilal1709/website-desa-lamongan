...
import { scanUploadedFile } from "@/lib/malware-scan"
...
export async function POST(request: Request) {
  ...
  if (file.size > 2 * 1024 * 1024) return Response.json({ message: "Ukuran CSV maksimal 2 MB." }, { status: 400 })
  try { await scanUploadedFile(file) } catch (error) { return Response.json({ message: error instanceof Error ? error.message : "CSV ditolak oleh pemindaian keamanan." }, { status: 422 }) }
  ...
  try { const result = await prisma.resident.createMany({ data: valid }); await audit("RESIDENT_IMPORT_COMPLETED", "RESIDENT", { actorId: access.user!.id, targetId: String(result.count), ip: clientAddress(request.headers) }); revalidateTag("home-data", "max"); revalidateTag("admin-dashboard", "max"); revalidatePath("/infografis"); revalidatePath("/admin"); await publishCmsUpdate("population"); return Response.json({ created: result.count, errors: [] }) }
