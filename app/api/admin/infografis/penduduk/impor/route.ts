...
export async function POST(request: Request) {
  ...
  try { const result = await prisma.resident.createMany({ data: valid }); revalidateTag("home-data", "max"); revalidateTag("admin-dashboard", "max"); revalidatePath("/infografis"); revalidatePath("/admin"); await publishCmsUpdate("population"); return Response.json({ created: result.count, errors: [] }) }
