...
export async function PATCH(request: Request, context: RouteContext<"/api/admin/access/[id]">) {
  ...
  try {
    ...
    if (body.kind === "role") {
      ...
      await prisma.adminSession.deleteMany({ where: { user: { roles: { some: { roleId: id } } } } }); return Response.json({ role })
    }
    if (body.kind === "user") {
      ...
      await prisma.adminSession.deleteMany({ where: { userId: id } }); return Response.json({ ok: true })
    }
...
export async function DELETE(request: Request, context: RouteContext<"/api/admin/access/[id]">) {
  ...
  if (kind === "role") { const role = await prisma.role.findUnique({ where: { id }, include: { _count: { select: { users: true } } } }); if (!role) return Response.json({ error: "Peran tidak ditemukan." }, { status: 404 }); if (role.isSystem || role._count.users) return Response.json({ error: "Peran sistem atau peran yang masih dipakai tidak dapat dihapus." }, { status: 400 }); await prisma.role.delete({ where: { id } }); return new Response(null, { status: 204 }) }
  const user = await prisma.adminUser.findUnique({ where: { id } }); if (!user) return Response.json({ error: "Akun tidak ditemukan." }, { status: 404 }); if (user.isSuperAdmin) return Response.json({ error: "Akun superadmin tidak dapat dihapus." }, { status: 400 }); await prisma.adminUser.delete({ where: { id } }); return new Response(null, { status: 204 })
}
