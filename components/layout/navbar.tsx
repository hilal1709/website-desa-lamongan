...
export function Navbar() {
  ...
  return <header className={`sticky top-0 z-50 border-b transition-all duration-300 ${scrolled || open ? "border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl" : "border-slate-100 bg-white/90 backdrop-blur"}`}>
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 lg:px-8">
      <Link href="/" className="flex items-center gap-2.5" aria-label="Beranda Desa Kedungrejo"><span className="grid size-9 place-items-center rounded-lg bg-emerald-800 text-white shadow-sm"><MapPinned size={19}/></span><span><b className="block text-sm leading-none text-emerald-950">DESA KEDUNGREJO</b><small className="mt-1 block text-[11px] font-medium text-slate-500">Kabupaten Banyuwangi</small></span></Link>
