...
export function PopulationEventManager() {
  ...
  async function saveBalance(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setToast({ message: "Saldo awal dan komposisi penduduk berhasil disimpan.", variant: "success" })
    } catch (error) {
      setToast({ message: error instanceof Error ? error.message : "Saldo awal tidak dapat disimpan.", variant: "error" })
  ...
  async function removeBalance(record: BalanceRecord) {
    if (!window.confirm(`Hapus saldo awal ${record.dusun}?`)) return
    setBusy(true)
    try {
      ...
      setToast({ message: "Saldo awal dihapus.", variant: "success" })
    } catch (error) {
      setToast({ message: error instanceof Error ? error.message : "Saldo awal tidak dapat dihapus.", variant: "error" })
    } finally { setBusy(false) }
  }
