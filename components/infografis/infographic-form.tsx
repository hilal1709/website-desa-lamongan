...
export function InfographicForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!client) {
      setSaving(false)
      setToast({ message: "Konfigurasi Supabase belum tersedia. Isi variabel lingkungan terlebih dahulu.", variant: "error" })
      return
    }
    ...
    if (!populationError) { const [ageDelete, educationDelete, occupationDelete] = await Promise.all([client.from("age_group_stats").delete().eq("year", year).eq("dusun", dusun), client.from("education_stats").delete().eq("year", year).eq("dusun", dusun), client.from("occupation_stats").delete().eq("year", year).eq("dusun", dusun)]); const detailError = ageDelete.error ?? educationDelete.error ?? occupationDelete.error; if (!detailError) { const [ageInsert, educationInsert, occupationInsert] = await Promise.all([client.from("age_group_stats").insert(ageRows), client.from("education_stats").insert(educationRows), client.from("occupation_stats").insert(occupationRows)]); const error = ageInsert.error ?? educationInsert.error ?? occupationInsert.error; if (!error) { const { data: yearStats, error: trendReadError } = await client.from("infographic_stats").select("total_population").eq("year", year); const trendTotal = (yearStats ?? []).reduce((sum, row) => sum + Number(row.total_population), 0); const { error: trendError } = trendReadError ? { error: trendReadError } : await client.from("population_trends").upsert({ year, total_population: trendTotal }, { onConflict: "year" }); if (!trendError) { setToast({ message: "Data infografis berhasil disimpan.", variant: "success" }); setSaving(false); return } } } }
  ...
  return <form onSubmit={submit} className="space-y-6 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-100 sm:p-7"><section><h2 className="text-lg font-bold text-slate-950">Data kependudukan</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-slate-700">Tahun<input required min="2000" max="2100" type="number" value={form.year} onChange={(event) => setForm((current) => ({ ...current, year: event.target.value }))} className={inputClass}/></label><label className="text-sm font-bold text-slate-700">Dusun<select required value={form.dusun} onChange={(event) => setForm((current) => ({ ...current, dusun: event.target.value }))} className={inputClass}><option value="" disabled>Pilih dusun</option>{hamlets.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-sm font-bold text-slate-700">Total Penduduk<input required min="0" type="number" value={form.population} onChange={(event) => field("population", event.target.value)} className={inputClass}/></label><label className="text-sm font-bold text-slate-700">Total Kepala Keluarga<input required min="0" type="number" value={form.households} onChange={(event) => field("households", event.target.value)} className={inputClass}/></label><label className="text-sm font-bold text-slate-700">Laki-laki<input required min="0" type="number" value={form.male} onChange={(event) => field("male", event.target.value)} className={inputClass}/></label><label className="text-sm font-bold text-slate-700">Perempuan<input required min="0" type="number" value={form.female} onChange={(event) => field("female", event.target.value)} className={inputClass}/></label></div></section><Group title="Kelompok usia" items={ageGroups} group="ages"/><Group title="Pendidikan" items={educationLevels} group="education"/><Group title="Mata pencaharian" items={occupations} group="occupations"/><div className="flex justify-end border-t border-slate-100 pt-5"><button disabled={saving} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-green-800 px-5 py-3 text-sm font-bold text-white hover:bg-green-900 disabled:opacity-70">{saving ? <LoaderCircle className="animate-spin" size={18}/> : <Save size={18}/>}{saving ? "Menyimpan..." : "Simpan data"}</button></div>{toast ? <Toast message={toast.message} variant={toast.variant}/> : null}</form>
}
