...
export function ResidentManager({ initialResidents }: { initialResidents: Resident[] }) {
    ...
    useEffect(() => { if (initialResidents.length) return; const timer = window.setTimeout(() => { void fetch("/api/admin/infografis/penduduk").then((response) => response.json().then((data) => response.ok ? setResidents(data.residents ?? []) : setMessage(data.message ?? "Profil warga belum dapat dimuat."))).catch(() => setMessage("Profil warga belum dapat dimuat.")); }, 0); return () => window.clearTimeout(timer); }, [initialResidents.length]);
