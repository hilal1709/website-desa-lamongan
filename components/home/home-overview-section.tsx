...
export function HomeOverviewSection({ pages, digitalItems, documents }: HomeOverviewSectionProps) {
  const profile = pages.get("profil")
  const services = pages.get("layanan")
  const populationStats = pages.get("data-desa")?.sections.find((section) => section.key === "stats")?.items ?? []
  const government = profile?.sections.find((section) => section.key === "government")?.items ?? []
  const serviceItems = services?.sections.find((section) => section.key === "service-cards")?.items ?? []

  return (
    <section aria-labelledby="village-overview-heading" className="mx-auto max-w-7xl px-5 py-16 sm:py-20">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card className="home-overview-card border-slate-200"><CardHeader><div className="flex items-center gap-3"><Landmark className="text-emerald-700" /><CardTitle>Pemerintahan & profil desa</CardTitle></div></CardHeader><CardContent className="space-y-4"><p className="leading-7 text-slate-600">{profile?.description}</p><div className="grid gap-2 sm:grid-cols-3">{government.slice(0, 3).map((item) => <div key={item.title} className="rounded-xl bg-slate-50 p-3"><p className="font-bold text-slate-800">{item.title}</p><p className="mt-1 text-sm text-slate-500">{item.description}</p></div>)}</div></CardContent></Card>
        <Card className="home-overview-card border-slate-200"><CardHeader><div className="flex items-center gap-3"><Landmark className="text-emerald-700" /><CardTitle>Layanan & aspirasi warga</CardTitle></div></CardHeader><CardContent className="space-y-4"><p className="leading-7 text-slate-600">{services?.description}</p><div className="flex flex-wrap gap-2">{serviceItems.map((item) => <span key={item.title} className="rounded-full bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-800">{item.title}</span>)}</div><p className="text-sm text-slate-500">Aduan warga dapat disampaikan dengan identitas yang dijaga dan ditindaklanjuti oleh desa.</p></CardContent></Card>
        <Card className="home-overview-card border-slate-200"><CardHeader><div className="flex items-center gap-3"><HeartPulse className="text-emerald-700" /><CardTitle>Data penduduk & kesehatan</CardTitle></div></CardHeader><CardContent><div className="grid grid-cols-2 gap-3">{populationStats.map((item) => <div key={item.title} className="rounded-xl bg-slate-50 p-3"><p className="text-2xl font-black text-slate-900">{item.value}</p><p className="mt-1 text-sm font-semibold text-slate-700">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.description}</p></div>)}</div><p className="mt-4 text-sm leading-6 text-slate-600">Program stunting memantau balita, risiko kesehatan, dan kehadiran posyandu secara berkala.</p></CardContent></Card>
        {documents.length > 0 && <Card className="home-overview-card border-slate-200"><CardHeader><div className="flex items-center gap-3"><Newspaper className="text-emerald-700" /><CardTitle>Berita & arsip publik</CardTitle></div></CardHeader><CardContent><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">Dokumen publik terbaru</p>{documents.map((item) => <p key={item.id} className="mt-3 text-sm font-semibold leading-6 text-slate-700">{item.title} <span className="font-normal text-slate-500">· {item.type} · {item.size}</span></p>)}</CardContent></Card>}
