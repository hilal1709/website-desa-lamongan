...
export function AdminDisasterManager() {
  ...
  useEffect(() => {
    let active = true
    void fetch("/api/bencana?includeInactive=1", { cache: "no-store" })
      .then(async (response) => { if (!response.ok) throw new Error("Pengaturan bencana tidak dapat dimuat."); return readApiResponse<{ setting?: { override?: typeof overrideStatus; announcement?: string | null }; locations?: DisasterLocation[] }>(response) })
      .then((data) => { if (!active) return; setOverrideStatus(data.setting?.override ?? "auto"); setAnnouncement(data.setting?.announcement ?? ""); setLocations(data.locations ?? []) })
      .catch((cause: unknown) => active && setError(cause instanceof Error ? cause.message : "Pengaturan bencana tidak dapat dimuat."))
