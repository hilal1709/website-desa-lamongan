...
import { DisasterAlertCircleIcon, DisasterCheckIcon, DisasterCloudRainIcon, DisasterInfoIcon, DisasterShieldCheckIcon, DisasterSparklesIcon, DisasterSproutIcon, DisasterSunIcon } from "@/components/bencana/disaster-icons"
import type { DisasterRiskLevel } from "@/components/bencana/disaster-types"
...
export function FarmingClimatePredictor({ precipitationSum, weatherCode, riskLevel }: ClimateProps) {
  ...
  const WeatherIcon = isRaining ? DisasterCloudRainIcon : DisasterSunIcon
  ...
  const getFarmingStatus = () => {
    if (riskLevel === "bahaya") {
      return {
        planting: { label: "Tunda Aktivitas Tanam", status: "bahaya", color: "bg-rose-50 text-rose-800 border-rose-300", icon: DisasterCloudRainIcon, desc: "Status siaga banjir ditetapkan admin desa. Tunda penanaman dan amankan bibit, pupuk, serta alat pertanian dari area rawan." },
        drying: { label: "Amankan Gabah & Panen", status: "bahaya", color: "bg-rose-50 text-rose-800 border-rose-300", icon: DisasterCloudRainIcon, desc: "Hentikan penjemuran. Pindahkan gabah dan hasil panen ke tempat tertutup yang aman dari hujan atau genangan." },
        residential: { label: "Siaga Banjir", status: "bahaya", color: "bg-rose-100 text-rose-950 border-rose-300", icon: DisasterAlertCircleIcon, desc: "Periksa drainase, siapkan dokumen penting dan kebutuhan darurat, serta ikuti arahan petugas desa." }
      }
    }
    if (riskLevel === "waspada") {
      return {
        planting: { label: "Tunda Tanam di Area Rawan", status: "waspada", color: "bg-amber-50 text-amber-800 border-amber-300", icon: DisasterAlertCircleIcon, desc: "Pantau kondisi saluran air dan hindari menanam di lahan yang berpotensi tergenang." },
        drying: { label: "Jemur dengan Kesiapsiagaan", status: "waspada", color: "bg-amber-50 text-amber-800 border-amber-300", icon: DisasterCloudRainIcon, desc: "Gunakan alas jemur yang mudah dipindahkan dan siapkan pelindung hasil panen." },
        residential: { label: "Waspada Genangan", status: "waspada", color: "bg-amber-50 text-amber-900 border-amber-300", icon: DisasterAlertCircleIcon, desc: "Bersihkan drainase rumah dan pantau informasi kesiapsiagaan dari pemerintah desa." }
      }
    }
    if (isHeavyRain) {
      return {
        planting: { label: "Waspada Luapan", status: "waspada", color: "bg-rose-50 text-rose-800 border-rose-200", icon: DisasterCloudRainIcon, desc: "Curah hujan sangat tinggi (>50mm). Hindari tanam bibit muda yang rawan hanyut." },
        drying: { label: "Tidak Disarankan", status: "bahaya", color: "bg-rose-50 text-rose-800 border-rose-200", icon: DisasterCloudRainIcon, desc: "Potensi hujan lebat. Amankan gabah & hasil panen ke dalam lumbung tertutup." },
        residential: { label: "Siaga Genangan", status: "bahaya", color: "bg-rose-100 text-rose-900 border-rose-300", icon: DisasterAlertCircleIcon, desc: "Waspada genangan air di kawasan persawahan & alur sungai dusun." }
      }
    }
    if (activeRain >= 15) {
      return {
        planting: { label: "Sangat Cocok Olah Tanah", status: "cocok", color: "bg-emerald-50 text-emerald-800 border-emerald-200", icon: DisasterSproutIcon, desc: "Pasokan air memadai antara 15 hingga 50 mm. Sangat baik untuk pembajakan sawah & olah lahan MT 1/2." },
        drying: { label: "Jemur Waspada", status: "waspada", color: "bg-amber-50 text-amber-800 border-amber-200", icon: DisasterSunIcon, desc: "Gunakan alas jemur cepat gulung. Potensi hujan ringan di siang/sore hari." },
        residential: { label: "Kondisi Waspada", status: "waspada", color: "bg-amber-50 text-amber-800 border-amber-200", icon: DisasterShieldCheckIcon, desc: "Pastikan saluran drainase depan rumah bersih dari sampah." }
      }
    }
    return {
      planting: { label: "Cukup Air / Perlu Irigasi", status: "normal", color: "bg-blue-50 text-blue-800 border-blue-200", icon: DisasterSproutIcon, desc: "Cuaca relatif cerah. Lakukan pengairan dari jaringan irigasi persawahan." },
      drying: { label: "Sangat Bagus Jemur", status: "bagus", color: "bg-emerald-50 text-emerald-800 border-emerald-200", icon: DisasterSunIcon, desc: "Sinar matahari maksimal. Ideal untuk penjemuran gabah, jagung & palawija." },
      residential: { label: "Aman Terkendali", status: "aman", color: "bg-emerald-50 text-emerald-800 border-emerald-200", icon: DisasterShieldCheckIcon, desc: "Cuaca cerah berawan. Aman untuk seluruh aktivitas warga desa." }
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div data-disaster-motion className="flex flex-col gap-2 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-xs font-extrabold text-emerald-800">
            <DisasterSparklesIcon className="h-3.5 w-3.5 text-amber-500" /> Prediksi Iklim Otomatis Petani
          </span>
      ...
      <div className="grid gap-5 sm:grid-cols-3">
        ...
        <div data-disaster-motion className={`rounded-3xl border p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 ${recommendations.planting.color}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/80 shadow-sm backdrop-blur-sm">
              <DisasterSproutIcon className="h-5 w-5" />
            </div>
          </div>
          ...
          <div className="mt-5 pt-3 border-t border-black/10 flex items-center justify-between text-xs font-extrabold">
            ...
            <DisasterCheckIcon className="h-4 w-4" />
          </div>
        </div>
        ...
        <div data-disaster-motion className={`rounded-3xl border p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 ${recommendations.drying.color}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/80 shadow-sm backdrop-blur-sm">
              <DisasterSunIcon className="h-5 w-5" />
            </div>
          </div>
        ...
        <div data-disaster-motion className={`rounded-3xl border p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 ${recommendations.residential.color}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/80 shadow-sm backdrop-blur-sm">
              <DisasterShieldCheckIcon className="h-5 w-5" />
            </div>
          </div>
      ...
      <div data-disaster-motion className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs font-medium text-slate-600">
        <DisasterInfoIcon className="h-5 w-5 shrink-0 text-emerald-700 mt-0.5" />
        <div>
