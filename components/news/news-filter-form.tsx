  ...
  return (
    <section aria-labelledby="news-filter-heading" className="news-filter rounded-3xl border border-emerald-100 bg-emerald-50/60 p-4 sm:p-5">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
        ...
        <p className="text-sm text-slate-500">Cari berdasarkan kata kunci atau kategori.</p>
      </div>
      <form role="search" className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_auto]" method="get">
        ...
        <label><span className="sr-only">Kategori berita</span><select name="kategori" defaultValue={category} className="h-11 w-full rounded-xl border border-emerald-100 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"><option value="">Semua kategori</option>{categories.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
