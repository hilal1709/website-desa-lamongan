...
export function UmkmManager({ initialBusinesses }: { initialBusinesses: ApiBusiness[] }) {
  const [businesses, setBusinesses] = useState<Business[]>(() => initialBusinesses.map(normalizeBusiness))
  ...
  const load = async (clearMessage = true) => { try { const response = await fetch("/api/cms/umkm"); const data = await readResponse<{ businesses?: ApiBusiness[] }>(response); if (response.ok) { setBusinesses((data.businesses ?? []).map(normalizeBusiness)); if (clearMessage) setMessage("") } else setMessage(data.message ?? "Data UMKM gagal dimuat.") } catch { setMessage("Koneksi ke server gagal. Silakan coba lagi.") } }
  useEffect(() => {
    // Daftar awal sudah tersedia pada respons halaman, sehingga tidak perlu
    // menunggu fetch kedua setiap kali pengguna berpindah modul.
    if (initialBusinesses.length) return
  ...
  const saveProfile = async () => {
    ...
    const data = await readResponse<{ business?: ApiBusiness }>(response)
    setSaving(false)
  ...
  const saveProduct = async () => {
    ...
    const next = product.id ? editing.products.map((item) => item.id === product.id ? savedProduct : item) : [...editing.products, savedProduct]
  ...
  const staleLoadMessage = message === "Data UMKM belum dapat dimuat. Silakan coba lagi." && (businesses.length > 0 || Boolean(editing))
  return <div className="space-y-5"><section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-emerald-700">Daftar usaha</p><h2 className="mt-1 text-xl font-black text-slate-950">UMKM terdaftar</h2></div><button type="button" onClick={() => { setEditing(null); setCreating(emptyProfile()); setProduct(null) }} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-800"><Plus className="size-4" />Tambah UMKM</button></div><div className="mt-5 divide-y divide-slate-100">{businesses.length ? visibleBusinesses.map((business) => <div key={business.id} className="flex items-center justify-between gap-4 py-4"><div className="flex min-w-0 items-center gap-3"><UmkmThumbnail src={business.logoUrl} name={business.name} /><div className="min-w-0"><p className="truncate font-bold text-slate-950">{business.name}</p><p className="mt-1 text-xs text-slate-500">{business.category} · {business.dusun} · {business.products.length} produk · {business.isPublished ? "Tayang" : "Disembunyikan"}</p></div></div><div className="flex shrink-0 gap-2"><button type="button" onClick={() => { setCreating(null); setEditing({ ...business, registeredAt: dateInput(business.registeredAt) }); setProduct(null) }} className="rounded-lg bg-slate-100 p-2 text-slate-700" aria-label={`Edit ${business.name}`}><Pencil className="size-4" /></button><button type="button" onClick={() => void removeBusiness(business.id)} className="rounded-lg bg-rose-50 p-2 text-rose-700" aria-label={`Hapus ${business.name}`}><Trash2 className="size-4" /></button></div></div>) : <p className="py-8 text-center text-sm font-medium text-slate-500">Belum ada UMKM. Tambahkan profil usaha pertama.</p>}</div><PaginationControls page={currentBusinessPage} totalPages={totalBusinessPages} totalItems={businesses.length} pageSize={businessesPerPage} onPageChange={setBusinessPage} itemLabel="UMKM" /></section>{current ? <section className="rounded-[28px] border border-emerald-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-emerald-700">Profil UMKM</p><h2 className="mt-1 text-xl font-black text-slate-950">{current.id ? `Edit ${current.name}` : "UMKM baru"}</h2></div><button type="button" onClick={() => { setCreating(null); setEditing(null); setProduct(null) }} className="rounded-xl p-2 text-slate-500 hover:bg-slate-100" aria-label="Tutup form"><X className="size-5" /></button></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold text-slate-700">Nama UMKM<input value={current.name ?? ""} onChange={(event) => { setField("name", event.target.value); if (!current.id) setField("slug", makeSlug(event.target.value)) }} className={input} /></label><label className="text-sm font-bold text-slate-700">Slug URL<input value={current.slug ?? ""} onChange={(event) => setField("slug", makeSlug(event.target.value))} className={input} placeholder="nama-umkm" /></label><label className="text-sm font-bold text-slate-700">Kategori usaha<input value={current.category ?? ""} onChange={(event) => setField("category", event.target.value)} className={input} placeholder="Contoh: Makanan & Minuman" /></label><label className="text-sm font-bold text-slate-700">Nomor WhatsApp<input value={current.whatsapp ?? ""} onChange={(event) => setField("whatsapp", event.target.value)} className={input} placeholder="08123456789" /></label><label className="text-sm font-bold text-slate-700">Dusun<input required value={current.dusun ?? ""} onChange={(event) => setField("dusun", event.target.value)} className={input} placeholder="Contoh: Dusun Gabang" /></label><label className="text-sm font-bold text-slate-700">Tanggal pencatatan<LegacyDatePicker required value={dateInput(current.registeredAt)} onChange={(event) => setField("registeredAt", event.target.value)} className={input} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">Deskripsi<textarea value={current.description ?? ""} onChange={(event) => setField("description", event.target.value)} className={input} rows={4} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">Alamat (opsional)<input value={current.address ?? ""} onChange={(event) => setField("address", event.target.value)} className={input} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">URL logo<input value={current.logoUrl ?? ""} onChange={(event) => setField("logoUrl", event.target.value)} className={input} /></label><div className="md:col-span-2"><CmsImageUpload onUploaded={(url) => setField("logoUrl", url)} /></div><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={current.isPublished ?? true} onChange={(event) => setField("isPublished", event.target.checked)} />Tayangkan profil UMKM</label></div><button type="button" disabled={saving} onClick={() => void saveProfile()} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white disabled:opacity-60"><Save className="size-4" />{saving ? "Menyimpan..." : "Simpan profil"}</button>{editing ? <ProductSection business={editing} product={product} setProduct={setProduct} saveProduct={saveProduct} removeProduct={removeProduct} saving={saving} /> : <p className="mt-5 rounded-2xl bg-amber-50 p-4 text-sm font-medium text-amber-900">Simpan profil terlebih dahulu sebelum menambahkan produk.</p>}</section> : null}{message && !staleLoadMessage ? <p className="inline-flex items-center gap-1.5 text-sm font-bold text-emerald-700"><Check className="size-4" />{message}</p> : null}</div>
}
...
function VariantEditor({ variants, onChange }: { variants: Variant[]; onChange: (variants: Variant[]) => void }) {
  const update = (index: number, field: keyof Variant, value: string | number) => onChange(variants.map((variant, current) => current === index ? { ...variant, [field]: value } : variant))
  return <div className="md:col-span-2 rounded-xl border border-slate-200 bg-white p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold text-slate-700">Varian & harga (opsional)</p><button type="button" onClick={() => onChange([...variants, { name: "", price: 0 }])} className="text-sm font-bold text-emerald-700">+ Tambah varian</button></div><div className="mt-3 space-y-2">{variants.map((variant, index) => <div key={index} className="grid grid-cols-[1fr_130px_auto] gap-2"><input value={variant.name} onChange={(event) => update(index, "name", event.target.value)} className={input} placeholder="Contoh: Original" /><input type="number" min="1" value={variant.price || ""} onChange={(event) => update(index, "price", Number(event.target.value))} className={input} placeholder="Harga" /><button type="button" onClick={() => onChange(variants.filter((_, current) => current !== index))} className="mt-1.5 rounded-xl bg-rose-50 px-3 text-sm font-bold text-rose-700">Hapus</button></div>)}</div><p className="mt-2 text-xs text-slate-500">Setiap varian dapat memakai harga sendiri. Bila kosong, produk memakai harga utama.</p></div>
}
...
function ProductSection({ business, product, setProduct, saveProduct, removeProduct, saving }: { business: Business; product: ProductForm | null; setProduct: (value: ProductForm | null) => void; saveProduct: () => void; removeProduct: (id: string) => void; saving: boolean }) {
  ...
  const perPage = 8
  const update = <K extends keyof ProductForm>(key: K, value: ProductForm[K] | unknown) => setProduct(product ? { ...product, [key]: value } as ProductForm : product)
  ...
  if (Boolean(product)) return <section className="mt-7 border-t border-slate-100 pt-6"><div className="flex items-center justify-between"><h3 className="text-lg font-black text-slate-950">{product!.id ? "Edit produk" : "Produk baru"}</h3><button type="button" onClick={() => setProduct(null)} className="text-sm font-bold text-slate-500">Batal</button></div><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold text-slate-700">Nama produk<input value={product!.name} onChange={(event) => update("name", event.target.value)} className={input} /></label><label className="text-sm font-bold text-slate-700">Harga utama<input type="number" min="1" value={product!.price || ""} onChange={(event) => update("price", Number(event.target.value))} className={input} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">Deskripsi<textarea value={product!.description} onChange={(event) => update("description", event.target.value)} className={input} rows={3} /></label><VariantEditor variants={(product!.variants ?? []) as Variant[]} onChange={(variants) => update("variants", variants)} /><label className="md:col-span-2 text-sm font-bold text-slate-700">URL gambar produk<input value={product!.imageUrl} onChange={(event) => update("imageUrl", event.target.value)} className={input} /></label><div className="md:col-span-2"><CmsImageUpload onUploaded={(url) => update("imageUrl", url)} /></div><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={product!.isAvailable} onChange={(event) => update("isAvailable", event.target.checked)} />Produk tersedia untuk dipesan</label></div><button type="button" disabled={saving} onClick={saveProduct} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save className="size-4" />Simpan produk</button></section>
  return <section className="mt-7 border-t border-slate-100 pt-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-emerald-700">Produk</p><h3 className="mt-1 text-lg font-black text-slate-950">Katalog {business.name}</h3></div><button type="button" onClick={() => setProduct(emptyProduct())} className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 px-4 py-2.5 text-sm font-bold text-emerald-800 hover:bg-emerald-50"><Plus className="size-4" />Tambah produk</button></div><div className="mt-4 divide-y divide-slate-100">{business.products.length ? visibleProducts.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 py-3"><div className="flex min-w-0 items-center gap-3"><img src={item.imageUrl} alt="" className="size-12 rounded-xl object-cover" /><div className="min-w-0"><p className="truncate font-bold text-slate-950">{item.name}</p><p className="text-xs text-slate-500">Rp{new Intl.NumberFormat("id-ID").format(item.price)} · {(item.variants?.length ?? 0) ? `${item.variants?.length} varian` : "Tanpa varian"} · {item.isAvailable ? "Tersedia" : "Nonaktif"}</p></div></div><div className="flex gap-2"><button type="button" onClick={() => setProduct(normalizeProduct(item))} className="rounded-lg bg-slate-100 p-2 text-slate-700" aria-label={`Edit ${item.name}`}><Pencil className="size-4" /></button><button type="button" onClick={() => removeProduct(item.id)} className="rounded-lg bg-rose-50 p-2 text-rose-700" aria-label={`Hapus ${item.name}`}><Trash2 className="size-4" /></button></div></div>) : <p className="py-5 text-sm text-slate-500">Belum ada produk.</p>}</div><PaginationControls page={currentPage} totalPages={totalPages} totalItems={business.products.length} pageSize={perPage} onPageChange={setPage} itemLabel="produk" />{product ? <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4"><div className="flex items-center justify-between"><h4 className="font-black text-slate-950">{product.id ? "Edit produk" : "Produk baru"}</h4><button type="button" onClick={() => setProduct(null)} className="text-sm font-bold text-slate-500">Batal</button></div><div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-sm font-bold text-slate-700">Nama produk<input value={product.name} onChange={(event) => update("name", event.target.value)} className={input} /></label><label className="text-sm font-bold text-slate-700">Harga (Rupiah)<input type="number" min="1" value={product.price || ""} onChange={(event) => update("price", Number(event.target.value))} className={input} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">Deskripsi produk<textarea value={product.description} onChange={(event) => update("description", event.target.value)} className={input} rows={3} /></label><label className="md:col-span-2 text-sm font-bold text-slate-700">Varian (opsional)<input value={(product.variants ?? []).join(", ")} onChange={(event) => update("variants", event.target.value.split(/[,\n]/).map((item) => item.trim()).filter(Boolean))} className={input} placeholder="Contoh: Original, Pedas, Keju" /><span className="mt-1 block text-xs font-medium text-slate-500">Pisahkan setiap pilihan dengan koma. Harga berlaku sama untuk semua varian.</span></label><label className="md:col-span-2 text-sm font-bold text-slate-700">URL gambar produk<input value={product.imageUrl} onChange={(event) => update("imageUrl", event.target.value)} className={input} /></label><div className="md:col-span-2"><CmsImageUpload onUploaded={(url) => update("imageUrl", url)} /></div><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={product.isAvailable} onChange={(event) => update("isAvailable", event.target.checked)} />Produk tersedia untuk dipesan</label></div><button type="button" disabled={saving} onClick={saveProduct} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save className="size-4" />Simpan produk</button></div> : null}</section>
}
