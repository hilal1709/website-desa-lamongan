...
export function ComplaintManager({ initialComplaints }: { initialComplaints: Complaint[] }) {
  ...
  const [loading, setLoading] = useState(initialComplaints.length === 0)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetch(`/api/cms/aduan?${params}`)
    ...
    } catch (error) { notify(error instanceof Error ? error.message : "Aduan tidak dapat dimuat.", "error") } finally { setLoading(false) }
  }, [categoryFilter, search, statusFilter])

  useEffect(() => {
    if (initialLoad.current) { initialLoad.current = false; if (initialComplaints.length) return }
    const timer = window.setTimeout(() => void load(), 120)
    return () => window.clearTimeout(timer)
  }, [initialComplaints.length, load])
