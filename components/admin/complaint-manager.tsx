...
export function ComplaintManager({ initialComplaints, initialPagination, initialStatusCounts }: { initialComplaints: Complaint[]; initialPagination: Pagination; initialStatusCounts: Record<ComplaintStatus, number> }) {
  ...
  const load = useCallback(async (nextPage: number) => {
    ...
    try {
      ...
      const payload = await result.json() as { complaints?: Complaint[]; message?: string; pagination?: Pagination; statusCounts?: Record<ComplaintStatus, number> }
...
function MobileComplaintCard({ item, onOpen }: { item: Complaint; onOpen: (item: Complaint) => void }) {
  return <article data-complaint-row className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="line-clamp-2 font-bold text-slate-950">{item.title}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{item.description}</p></div><StatusBadge tone={statusTone[item.status]}>{item.status}</StatusBadge></div><div className="mt-4 grid gap-2 border-y border-slate-100 py-3 text-xs text-slate-600"><p className="flex items-center gap-2"><HugeiconsIcon icon={FileTextIcon} className="size-3.5 text-emerald-700" />{item.category}</p><p className="flex items-center gap-2"><HugeiconsIcon icon={MapPinIcon} className="size-3.5 text-emerald-700" />{item.location}</p><p className="flex items-center gap-2"><HugeiconsIcon icon={Calendar01Icon} className="size-3.5 text-emerald-700" />{dateFormatter.format(new Date(item.createdAt))}</p></div><Button data-complaint-interactive size="sm" variant="outline" className="mt-3 w-full" onClick={() => onOpen(item)}><HugeiconsIcon icon={ViewIcon} />Tinjau aduan</Button></article>
}
