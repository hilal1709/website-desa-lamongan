...
const menu: { icon: LucideIcon; label: string; href: string; module: CmsModule }[] = [
  ...
  { icon: FileText, label: "Layanan & Pengajuan", href: "/admin/layanan", module: "SERVICE_CATALOG" }, { icon: FileText, label: "Antrian Layanan", href: "/admin/antrian", module: "SERVICE_SUBMISSIONS" }, { icon: MessageSquare, label: "Kelola Aduan", href: "/admin/aduan", module: "COMPLAINTS" }, { icon: MessageSquare, label: "Modul CMS", href: "/admin/modul", module: "CMS_MODULES" }, { icon: Settings, label: "Pengaturan", href: "/admin/pengaturan", module: "SETTINGS" },
]
...
export function AdminSidebar({ user }: { user: CurrentAdmin }) {
  ...
  return <>
    ...
    <aside data-admin-sidebar className="hidden w-[280px] rounded-b-[28px] border border-emerald-900/10 bg-slate-950 p-4 text-white shadow-xl shadow-emerald-950/10 lg:fixed lg:inset-y-0 lg:left-[max(1.5rem,calc((100vw-1500px)/2))] lg:z-30 lg:flex lg:h-svh lg:flex-col">
      ...
      <nav aria-label="Navigasi CMS" className="mt-6 min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain pr-1 [scrollbar-color:rgba(148,163,184,.55)_transparent] [scrollbar-width:thin]">{permittedMenu.map(({ icon: Icon, label, href }) => { const active = isActive(href); return <Link key={href} href={href} className={`flex items-center justify-between rounded-2xl px-3 py-3 text-sm font-bold transition ${active ? "bg-emerald-400 text-slate-950" : "text-slate-300 hover:bg-white/8 hover:text-white"}`}><span className="flex items-center gap-3"><Icon className="h-4 w-4" />{label}</span>{active && <ChevronRight className="h-4 w-4" />}</Link> })}{user.isSuperAdmin ? <Link href="/admin/akses" className="flex items-center gap-3 rounded-2xl px-3 py-3 text-sm font-bold text-slate-300 hover:bg-white/8 hover:text-white"><Settings className="h-4 w-4" />Akun & Hak Akses</Link> : null}</nav>
